...
$sql = "SELECT * FROM `admin` WHERE `deleted` IS NULL ORDER BY `username` DESC LIMIT $start_from, $limit";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        //$post_id = $row['post_id'];
        $sql2 = "SELECT COUNT(name) FROM `admin`";
...
//	echo'<div class="panel-group"><div class="panel panel-info">';
	...
	?><div class="card-body">
				<h3 class="card-title">Name: <span style="color:blue"><?php echo $row['name']; ?></span></h3>
                <h4 class="card-title">Username: <span style="color:blue"><?php echo $row['username']; ?></span></h4>
