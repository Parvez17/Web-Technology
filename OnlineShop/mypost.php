...
if (isset($_GET["page"])) { $page  = $_GET["page"]; } else { $page=1; };
$start_from = ($page-1) * $limit;
$sql = "SELECT * FROM `post` WHERE `post_deleted` IS NULL and `username`='".$_SESSION["user"]."' ORDER BY `post_id` DESC LIMIT $start_from, $limit";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $post_id = $row['post_id'];
        $sql2 = "SELECT * FROM `post_pic` WHERE `post_id` ='" . $row['post_id'] . "'";
        $result2 = mysqli_query($conn, $sql2);
...
?>
               ...
               <!-- Pager  -->
				...
				<nav aria-label="Page navigation example">
				  <ul class="pagination justify-content-center">
				    <?php
        $sql = "SELECT COUNT(post_id) FROM `post` where username='".$_SESSION["user"]."'";
        $rs_result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_row($rs_result);
        $total_records = $row[0];
        $total_pages = ceil($total_records / $limit);
        $pagLink = "<ul class='pagination'>";
        for ($i=1; $i<=$total_pages; $i++) {
            $pagLink .= "<li class='page-item'><a class='page-link' href='mypost.php?page=".$i."'>".$i."</a></li>";
        };
        echo $pagLink . "</ul>";
		?>
        <!--
