...
// gender validation
  ...
  $sql = "INSERT INTO user (username, name, email,password,gender,phone_number) VALUES ('$fname', '$uname', '$email','$pwd','$gender','$phone')";

  if($err==1){
   if ($pwd==$cpwd) {
      if (mysqli_query($conn, $sql)) {
        if (!isset($_COOKIE['fullname']) && !isset($_COOKIE['email']) && !isset($_COOKIE['phoneNum']) && !isset($_COOKIE['pass']) && !isset($_COOKIE['cnfpass']) ) {
           // $fname=$uname=$email=$phone=$pwd=$cpwd=$gender="";
            setcookie("fullname",$fname,time()+20);
            setcookie("email",$email,time()+20);
            setcookie("phoneNum",$phone,time()+20);
         ...
         <div class="form-group">
                ...
                if (isset($_COOKIE['fullname'])) {
                  echo '<label for="fname">Fullname </label>
                      ...
                      echo "hlwwwwwwwwwwwwwwwwwwwwwwwww";
                }else{
                  echo '<label for="fname">Fullname </label>
          ...
          <div class="form-group">
            ...
            <input class="form-control" type="tel" id="phone" name="phone" placeholder="01xxxxxxxxx" pattern="[0-9]{11}"  />
            <?php if(!empty($errphone)){echo "<span style='background:red;color: white;padding: 3px;border-radius:5px;'> $errphone</span>";}?>
          ...
          <div id="gender" class="form-group">
                ...
                <?php if(!empty($errgender)){echo "<span style='background:red;color: white;padding: 3px;border-radius:5px;'> $errgender</span>";}?>
            </div>
